...
const DEFAULT_GLOW_COLOR = '0, 255, 65'; // Ton vert habituel
...
const ParticleCard = ({
  ...
  useEffect(() => {
    ...
    const handleClick = e => {
      ...
      ripple.style.cssText = `
        ...
        left: ${x - 100}px; top: ${y - 100}px; pointer-events: none; z-index: 1000;
      `;
      element.appendChild(ripple);
      gsap.fromTo(ripple, { scale: 0, opacity: 1 }, { scale: 3, opacity: 0, duration: 0.8, onComplete: () => ripple.remove() });
    };
  ...
  return (
    <div ref={cardRef} className={`${className} particle-container relative overflow-hidden`}>
      {children}
    </div>
  );
};
...
const GlobalSpotlight = ({ gridRef, disableAnimations, enabled, spotlightRadius, glowColor }) => {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = e => {
      ...
      gsap.to(spotlightRef.current, { left: e.clientX, top: e.clientY, opacity: minDistance <= proximity ? 0.8 : (minDistance <= fadeDistance ? (fadeDistance - minDistance) / (fadeDistance - proximity) * 0.8 : 0), duration: 0.1 });
    };
      ...
      <div className="card-grid bento-section" ref={gridRef}>
        {STACK.map((card, index) => {
          ...
          const cardProps = {
            ...
            style: { '--glow-color': glowColor, '--card-bg': '#0a0a0a' }
          };
          ...
          return (
            ...
            >
              <div className="magic-bento-card__header">
                <div className="magic-bento-card__label">{card.category}</div>
              </div>
